...
pygame.display.set_caption("Warehouse Robot PPO (10x10 Test)")
...
while running and step_count < 100:
    ...
    with torch.no_grad():
        ...
        action = torch.argmax(probs).item()  # 또는 샘플링 가능

    next_state, reward, done = env.step(actions[action])
    draw_grid(env.agent_pos, env.has_item)
    print(f"Step {step_count} 위치: {env.agent_pos}, 액션: {actions[action]}, 보상: {reward}")
    state = next_state
    step_count += 1
